#!/usr/bin/env python
# from numpy import * # This line is removed to prevent function name conflicts.
import numpy as np
import os
import glob
import sys
...
def get_prior_shift(t_start, t_end, bins_histogram):
    ...
    sampled_K = []
    # Gamma hyper-prior
    G_shape = 2.0  # currently hard-coded
    G_rate = 1.0  # mode at 1
    ...
    for rep in range(100000):
        ...
        try:
            # Poisson prior
            Poi_lambda = np.random.gamma(G_shape, 1.0 / G_rate)
            n_rates_temp = np.random.poisson(Poi_lambda, 1000)
            n_rates = n_rates_temp[n_rates_temp > 0][0]
            shift_times = list(
                np.random.uniform(
                    t_end - min_time_frame_size,
                    t_start + min_time_frame_size,
                    n_rates - 1,
                )
            )
...
def get_marginal_rates(f_name, start_age, end_age, nbins=0, burnin=0.2):
    ...
    for i in range(burnin, len(post_rate)):
        ...
        if len(row) == 1:
            ...
            ind_rates = np.arange(0, int(np.ceil(len(row) / 2.0)))
            ind_shifts = np.arange(int(np.ceil(len(row) / 2.0)), len(row))
            rates = row[ind_rates]
    ...
    time_frames = bins_histogram - abs(bins_histogram[1] - bins_histogram[0]) / 2.0
    time_frames = time_frames[1:]
    n_mcmc_samples = len(post_rate) - burnin
    return [
        time_frames,
        mean_rates,
        np.array(min_rates),
        np.array(max_rates),
        np.array(times_of_shift),
        n_mcmc_samples,
        marginal_rates_list,
    ]
...
def get_r_plot(
    ...
    if plot_log == 0:
        ...
        out_str += "\npolygon(c(time, rev(time)), c({0}_maxHPD, rev({0}_minHPD)), col = alpha('{1}',0.3), border = NA)".format(
            prefix, col
        )
        ...
        out_str += (
            ...
            % (
                np.nanmin(np.log10(0.9 * res[2])),
                np.nanmax(np.log10(1.1 * res[3])),
                minXaxis,
                maxXaxis,
                parameter,
                time_lab,
        ...
        out_str += "\npolygon(c(time, rev(time)), c(log10({0}_maxHPD), rev(log10({0}_minHPD))), col = alpha('{1}',0.3), border = NA)".format(
            prefix, col
        )
        out_str += "\nlines(time,log10({0}_rate), col = '{1}', lwd=2)".format(
            prefix, col
        )

    bins_histogram = np.arange(max_age, min_age)
    if len(res[4]) > 1:
        h = np.histogram(res[4], bins=bins_histogram)
    else:
    ...
    out_str += (
        ...
        % (
            ...
            minXaxis,
            maxXaxis,
            max(np.max(h[0] / float(res[5])), 0.2),
            time_lab,
            col,
        )
    )
    ...
    counts_bf2[counts_bf2 != 1] = None
    counts_bf2 = counts_bf2 * res[1][::-1]
    counts_bf6[counts_bf6 != 1] = None
    counts_bf6 = counts_bf6 * res[1][::-1]
...
def pretty_ggplot(resS, resE, div_log):
    ...
    if np.min(tbl[:, 2]) > 0:
        emp_birth = tbl[:, 0] / tbl[:, 2]
        emp_birth[0] = None
        emp_death = tbl[:, 1] / tbl[:, 2]
        emp_death[0] = None

        out_str = "\n\n#PRETTY PLOT\n"
        # FIX: The usePackage function is modified to include a default repository.
...
def plot_net_rate(resS, resE, col, min_age, max_age, nbins, plot_title, burnin=0.2):
    resS_marginal_rate = resS[6]
    resE_marginal_rate = resE[6]
    max_indx = np.min([resS_marginal_rate.shape[0], resE_marginal_rate.shape[0]])
    marginal_rates_list = (
        resS_marginal_rate[0:max_indx, :] - resE_marginal_rate[0:max_indx, :]
    )
    mean_rates = np.mean(marginal_rates_list, axis=0)
    min_rates, max_rates = [], []
    for i in range(nbins):
    ...
    out_str += (
        ...
        % (
            min(0, 1.1 * np.nanmin(min_rates)),
            1.1 * np.nanmax(max_rates),
            minXaxis,
            maxXaxis,
            time_lab,
            plot_title,
...
def combine_logs(mcmc_files, wd, burnin_pct):
    ...
    with open(wd + "/COMBINED_sp_rates.log", "w") as o:
        o.writelines(total_log)
    total_log = []
    for file_name in mcmc_files:
        ...
        with open(file_name) as f:
            ...
            burnin = int(burnin_pct * len(file_log))
            total_log += file_log[burnin:]
    with open(wd + "/COMBINED_ex_rates.log", "w") as o:
        o.writelines(total_log)
    sp_events = []
    ex_events = []
    br_length = []
    for file_name in mcmc_files:
        ...
        sp_events.append(div[:, 0])
        ex_events.append(div[:, 1])
        br_length.append(div[:, 2])
    sp_events = np.mean(np.array(sp_events), axis=0)
    ex_events = np.mean(np.array(ex_events), axis=0)
    br_length = np.mean(np.array(br_length), axis=0)
    combined_div = pd.DataFrame(
        {"sp_events": sp_events, "ex_events": ex_events, "br_length": br_length}
    )
    combined_div[["sp_events", "ex_events", "br_length"]].to_csv(
        wd + "/COMBINED_div.log", sep="\t", index=False
    )
...
def plot_marginal_rates(
    ...
    if wd + "/COMBINED_mcmc.log" in files:
        files.remove(wd + "/COMBINED_mcmc.log")
    ...
    # FIX: Define the robust usePackage function once, at the start of the R script.
    r_str += "usePackage <- function(p){if (!is.element(p, installed.packages()[,1])) install.packages(p, dep = TRUE, repos='https://cloud.r-project.org'); library(p, character.only = TRUE)}\n"
    r_str += "usePackage('scales')\n"
    ...
    for mcmc_file in files:
        try:
            ...
            colors = ["#4c4cec", "#e34a33", "#32CD32"]
            r_str += "\n###%s###\n" % name_file
            r_str += get_K_values(tbl, head, colors[0], "l", burnin=burnin)

            f_name_sp = mcmc_file.replace("mcmc.log", "sp_rates.log")
            resS = get_marginal_rates(
                f_name_sp, min_age_t, max_age_t, nbins, burnin=burnin
            )
            spec_str, BFs = get_r_plot(
                resS,
                col=colors[0],
                parameter="Birth rate",
                min_age=min_age_t,
                max_age=max_age_t,
                plot_title=name_file,
                plot_log=logT,
            )
            r_str += spec_str

            f_name_ex = mcmc_file.replace("mcmc.log", "ex_rates.log")
            resE = get_marginal_rates(
                f_name_ex, min_age_t, max_age_t, nbins, burnin=burnin
            )
            ...
            f_name_div = mcmc_file.replace("mcmc.log", "div.log")
            r_str += plot_net_diversity(
                f_name_div,
                resS,
                col=colors[2],
                min_age=min_age_t,
            ...
            r_str += pretty_ggplot(resS, resE, f_name_div)

        except Exception as e:
            print(f"Could not read or process file: {mcmc_file}\nError: {e}")
...
p.add_argument(
    "input_data",
    metavar="<path to log files>",
    type=str,
    help="Input python file - see template",
    default="",
)
p.add_argument(
    "-combine",
    metavar="0",
    type=int,
    help="Whether log files should be combined before plotting or plotted serially",
)
p.add_argument(
    "-logT", metavar="1", type=int, help="set to 1 to log transform rates", default=0
)
p.add_argument(
    "-burnin", metavar=".2", type=float, help="set as a fraction", default=0.2
)
p.add_argument(
    "-TBP", help="Default is AD. Include for TBP.", default=False, action="store_true"
)
...
TBP = args.TBP
plot_marginal_rates(
    path_dir_log_files, logT=args.logT, burnin=args.burnin, combine=args.combine
)
